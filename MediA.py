...
async def get_sub_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=btn_text, url=sub_link, style=btn_style)]
    ])

async def get_force_sub_keyboard() -> InlineKeyboardMarkup:
    sub_link = await get_setting("sub_link", DEFAULT_SUBSCRIBE_LINK)
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="اشترك بالقناة", url=sub_link, style="primary")]
    ])
        ...
        if trigger_emoji_logic and modification_count == 2:
            async def trigger_emoji_msg():
                ...
                if len(active_emoji_tasks[chat_id]) > 3:
                    ...
                    oldest_task.cancel()
            asyncio.create_task(trigger_emoji_msg())
...
async def queue_worker():
    while True:
        ...
        status_msg = await live_typing_reply(message, proc_msg, reply_markup=sub_kb)
        file_path = None
            ...
            if res and res[0] and os.path.exists(res[0]):
                ...
                if is_audio:
                    ...
                    audio_msg = await message.reply_audio(
                        ...
                        caption=BotMessages.SUCCESS_AUDIO, 
                        title=new_title,
                        reply_markup=sub_kb
                    )
                    ...
                    if audio_msg and audio_msg.audio:
                        async with aiosqlite.connect("bot_data.db") as db:
                            await db.execute(
                                "INSERT OR REPLACE INTO media_cache (media_key, file_id, media_type, title) VALUES (?, ?, ?, ?)",
                                (cache_key, audio_msg.audio.file_id, "audio", new_title)
                            )
                            await db.commit()
                    ...
                    video_msg = await message.reply_video(
                        ...
                        caption=BotMessages.SUCCESS_VIDEO,
                        reply_markup=sub_kb
                    )
                    ...
                    if video_msg and video_msg.video:
                        async with aiosqlite.connect("bot_data.db") as db:
                            await db.execute(
                                "INSERT OR REPLACE INTO media_cache (media_key, file_id, media_type, title) VALUES (?, ?, ?, ?)",
                                (cache_key, video_msg.video.file_id, "video", orig_title)
                            )
                            await db.commit()
                ...
                await live_typing_reply(message, BotMessages.NOT_FOUND, reply_markup=sub_kb)
        except Exception:
            if status_msg:
                await status_msg.delete()
            await live_typing_reply(message, BotMessages.NOT_FOUND, reply_markup=sub_kb)
...
async def universal_handler(message: Message):
    ...
    if message.text and message.text.strip() == "يوت":
        if not is_group or (is_group navigate and await is_user_admin_or_owner(chat_id, user_id, force_update=True)):
            if not await check_force_subscription(user_id):
                force_kb = await get_force_sub_keyboard()
                await live_typing_reply(message, BotMessages.FORCE_SUB, reply_markup=force_kb)
                return
...
async def send_startup_notification():
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(chat_id=admin_id, text=BotMessages.STARTUP_MSG)
        except Exception:
            pass
